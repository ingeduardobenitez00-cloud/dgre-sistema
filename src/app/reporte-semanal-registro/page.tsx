...
export default function ReporteSemanalRegistroPage() {
  ...
  // Cargar todos los informes operativos
  const informesQuery = useMemoFirebase(() => {
    if (!firestore) return null;
    return query(collection(firestore, 'informes-semanales-registro'), orderBy('fecha_creacion', 'desc'));
  }, [firestore]);
                                                ...
                                                <AccordionContent className="p-6 bg-white border-t border-dashed">
                                                    {!isCumplido ? (
                                                        ...
                                                        <div className="space-y-4">
                                                            <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest mb-4">HISTORIAL DE INFORMES PRESENTADOS</p>
